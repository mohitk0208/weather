...
const TodayWeather = () => {
  ...
  return (
    <div className={`w-full px-8 md:px-10 pb-10 pt-36  flex justify-between bg-transparent`}>

      <div className="text-white">

        <div className=" text-5xl md:text-6xl relative pb-2 max-w-max">
          <span className="">
            {unit ?
              current ? current.main.temp : "__.__"
              : current ? celsiusToFahrenheit(current.main.temp) : "__.__"}
          </span>
          <span className="text-3xl md:text-4xl absolute top-0 left-full ">&deg;{unit ? "C" : "F"}</span>
        </div>
      ...
      <div className="text-white">
        <div className="text-4xl md:text-5xl pb-2 md:pb-3">
